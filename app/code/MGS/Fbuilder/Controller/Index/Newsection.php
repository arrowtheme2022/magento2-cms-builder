    ...
    public function execute()
    {
		if($this->getRequest()->isAjax() && ($this->customerSession->getUseFrontendBuilder() == 1)){
			...
			if($pageId = $this->getRequest()->getParam('page_id')){
				...
				$collection = $this->getModel('MGS\Fbuilder\Model\Section')->getCollection()->addFieldToFilter('page_id', $pageId);
				if(count($collection)==6){
					return $this->getResponse()->setBody(count($collection));
				}
			}
			try {
				$section->save();
								...
								<div class="line content-panel empty-block">
									<div class="add-new-block col-des-12">';
										...
										if($this->getRequest()->getParam('page_id')){
											$url = $this->_urlBuilder->getUrl('fbuilder/create/block', ['page_id'=>$pageId,'name'=>$name.'-0']);
											$html .= '<button class="action" type="button" onclick="openPopup(\''.$url.'\')" data-page-id="'.$pageId .'" data-block-name="'.$name.'-0"><em class="fa fa-plus-circle"></em>&nbsp;'.__('Add New Block').'</button>';
											
											if($this->customerSession->getBlockCopied()){
												$html .= '&nbsp;<button class="action btn btn-default btn-dulicate" type="button" onclick="pasteBlock('.$pageId.', \''.$name.'-0\')"><em class="fa fa-plus-circle"></em>&nbsp;'.__('Paste Copied Block').'</button>';
											}
										}else{
